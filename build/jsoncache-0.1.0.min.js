...
function(b){if(typeof b!=="number")throw Error("Cannot update cache total size without a char count");var a=parseInt(window.localStorage["JSONCache size"],10);isNaN(a)&&(a=0);b=a+b*2;b<=0?window.localStorage.removeItem("JSONCache size"):window.localStorage["JSONCache size"]=b},m=function(b,a){var c=JSON.stringify(a),d=c.length,e=f._getTime(),h=function(){if(f.getCacheSize()+d*2>g.maxCacheSize)throw Error("Cache add would exceed maxCacheSize ("+(f.getCacheSize()+d*2)+" > "+g.maxCacheSize+")");try{window.localStorage["JSONCache data "+
